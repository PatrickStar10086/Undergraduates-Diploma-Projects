					...
					<div class="col-sm-12">
						<section class="panel">
							<header class="panel-heading ">
								用户管理 <span class="tools pull-right"> <a
									...
									<c:if test="${ !empty userall}">
										...
										<c:forEach items="${ userall}" var="item">
											...
											<tr>
												...
												<td>
													<!--  定义修改用户按钮和删除用户按钮，修改提交地址为updateuser.action?id=${item.id}，删除提交地址为 deleteuser.action?id=${item.id}-->

													<a href="updateuser.action?id=${item.id}">修改</a> <a
													href="deleteuser.action?id=${item.id}">删除</a>
