					...
					<div class="col-sm-12">
						<section class="panel">
							<header class="panel-heading ">
								管理员管理 <span class="tools pull-right"> <a
									...
									<c:if test="${ !empty adminall}">
										<c:forEach items="${ adminall}" var="item">
											<tr>
												...
												<td>${item.password }</td>
												<td>
													<!--  定义修改管理员按钮和删除管理员按钮，修改提交地址为updateadmin.action?id=${item.id}，删除提交地址为 deleteadmin.action?id=${item.id}-->

													<a href="updateadmin.action?id=${item.id}">修改</a> <a
													href="deleteadmin.action?id=${item.id}">删除</a>
