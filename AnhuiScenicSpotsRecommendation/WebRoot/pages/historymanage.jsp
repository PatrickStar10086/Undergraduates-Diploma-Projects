					...
					<div class="col-sm-12">
						<section class="panel">
							<header class="panel-heading ">
								收藏管理 <span class="tools pull-right"> <a
							...
							<table class="table convert-data-table data-table">
								...
								<tbody>
									...
									<c:if test="${ !empty historyall}">

										<c:forEach items="${ historyall}" var="item">



											<tr>
												<td>${item.scenicspot }</td>
												<td>${item.user }</td>
												<td>
													<!--  定义修改收藏按钮和删除收藏按钮，修改提交地址为updatehistory.action?id=${item.id}，删除提交地址为 deletehistory.action?id=${item.id}-->

													<a href="updatehistory.action?id=${item.id}">修改</a> <a
													href="deletehistory.action?id=${item.id}">删除</a>
